...
async function load(){
  ...
  state.students=state.students.filter(s=>{
    if(s.status==="expelled" || !classIds.has(s.classId)) return false;
    const assignmentsForStudent=state.assignments.filter(a=>a.classId===s.classId && (!a.sectionId || !s.sectionId || a.sectionId===s.sectionId));
    return assignmentsForStudent.length>0;
  });
